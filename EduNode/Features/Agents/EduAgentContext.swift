...
struct EduAgentSuggestedPromptsResponse: Decodable {
    let suggestions: [String]
}

struct EduAgentThinkingPlanResponse: Decodable {
    let decisionMode: String
    let thinkingTraceMarkdown: String
...
struct EduAgentCourseSnapshot: Encodable {
    let name: String
    let subject: String
...
struct EduAgentNodeFieldSnapshot: Encodable {
    let id: String
    let label: String
    let value: String
}

struct EduAgentGraphNodeSnapshot: Encodable {
    let id: String
    let nodeType: String
    ...
    let textFields: [EduAgentNodeFieldSnapshot]
    let optionFields: [EduAgentNodeFieldSnapshot]
}

struct EduAgentGraphConnectionSnapshot: Encodable {
...
struct EduAgentSlideSnapshot: Encodable {
    let slideID: String
    let title: String
    let subtitle: String
...
struct EduAgentSupportedCanvasMethodSnapshot: Encodable {
    let methodID: String
    let title: String
    let textFields: [EduAgentNodeFieldSnapshot]
    let optionFields: [EduAgentNodeFieldSnapshot]
...
struct EduAgentSupportedCanvasNodeSnapshot: Encodable {
    let nodeType: String
    let title: String
    let methods: [EduAgentSupportedCanvasMethodSnapshot]
    let directTextEditable: Bool
    let directSelectableOptions: [String]
}

struct EduAgentWorkspaceSnapshot: Encodable {
    let course: EduAgentCourseSnapshot
    let nodes: [EduAgentGraphNodeSnapshot]
    let connections: [EduAgentGraphConnectionSnapshot]
    let slides: [EduAgentSlideSnapshot]
    let lessonPlanMarkdown: String?
}

struct EduAgentCanvasSchemaSnapshot: Encodable {
    let nodes: [EduAgentSupportedCanvasNodeSnapshot]
}
