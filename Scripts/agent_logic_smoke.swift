...
struct AgentLogicSmoke {
    static func main() throws {
        ...
        if let templatePath = ProcessInfo.processInfo.environment["EDUNODE_REFERENCE_TEMPLATE_PATH"],
           !templatePath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            let realTemplateURL = URL(fileURLWithPath: templatePath)
            if FileManager.default.fileExists(atPath: realTemplateURL.path) {
                ...
                print("REAL_TEMPLATE | skipped because EDUNODE_REFERENCE_TEMPLATE_PATH does not exist")
            }
        } else {
            print("REAL_TEMPLATE | skipped because EDUNODE_REFERENCE_TEMPLATE_PATH is not set")
        }
