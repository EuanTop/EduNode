...
- 后端服务：`Server/` 下的 Python/FastAPI `EduNodeServer`，保持前端 API 契约不变，同时更方便做多服务商 LLM 路由。
- 图谱引擎：通过 Swift Package Manager 集成 GNodeKit。
- LLM 接入：工作区 Agent 已改为通过 `EduNodeServer` 调用，后端使用 LiteLLM SDK 处理 provider 归一、多模型 fallback、token usage 与 Langfuse tracing。
- 参考教案解析：通过 MinerU 完成，但只由后端调用，并统一配置在 `Server/.env`。
- 产出链路：从节点图生成教案与展示材料，支持 Markdown、HTML、PDF 等格式。
- 第一阶段交付策略 - service-oriented monolith：考虑到云端成本与部署复杂度，鉴权、Agent 编排、基于 LiteLLM 的路由、Langfuse 观测钩子、本地用量日志与参考教案解析统一放在一个 Python 后端服务中。内部边界仍然保持清晰，后续流量增长后可以再拆分。
...
客户端现在只通过 `EduNodeServer` 完成账户登录。Supabase 被收口在 Python 后端内部，前端不再携带 Supabase URL 或 publishable key 配置。
...
服务器部署时，真实配置可以放在被忽略的 `Server/.env.dev` 或 `Server/.env.production`，再通过运行环境里的 `EDUNODE_ENV=dev` 或 `EDUNODE_ENV=production` 选择。
...
如需配置多个 LLM 服务商，请复制 `Server/llm_models.example.json` 为 `Server/llm_models.json`，按 priority 添加模型。后端会通过 LiteLLM SDK 按优先级尝试 enabled 模型，归一不同 provider 的返回格式，并把调用用量写入 `EDUNODE_USAGE_LOG_PATH` 指向的 JSONL 文件。若配置了 Langfuse keys，LiteLLM 还会通过 `langfuse_otel` callback 上报 trace。
